...
select   'SHARED POOL' "SGA POOLS"
,        '-->' || SH_SZ || ' M' "MEMORY"
,        '' "AVG"
,        '' "#"
,        'OTHER POOLS' "OTHER POOLS"
,        '-->' || OP_SZ || ' M' "MEMORY"
from     ( select round(sum(BYTES)/1024/1024) SH_SZ
	   from gv$sgastat
	   where POOL ='shared pool')
,        ( select round(sum(BYTES)/1024/1024) OP_SZ
	   from gv$sgastat
	   where POOL !='shared pool' or POOL IS NULL)
UNION ALL
...
,        B.POOL
,        B.SZ_
from     ( select   ROWNUM ROW_
           ,        POOL
           ,        PCT
           ,        decode( sign( length(SIZE_)-7)
                          , 1, round(SIZE_/1024/1024)||' M'
                          , '   '||round(SIZE_/1024)||' K' ) SZ_
           from     ( select   NAME POOL ,BYTES SIZE_ ,round(BYTES/SUM_*100)||'%' PCT
                      from     gv$sgastat
                      ,        ( select sum(BYTES) SUM_ from gv$sgastat where POOL ='shared pool' )
                      where    POOL = 'shared pool'
                      order by 2 desc )
           where      rownum < 7 ) SP
----------------------------------------------------------------------
...
from     DUAL
UNION ALL
select   'ALL UGAs AND PGAs','','','','',''
from     DUAL
UNION ALL
select   'UGA / PGA ...','MEMORY','AVG','','ESTIMATED MAX SIZE',''
from     DUAL
UNION ALL
